...
describe('holding.ts initHoldings', () => {
  beforeEach(() => {
  ...
  test('initHoldings 有数据', async () => {
    mockGetHoldings.mockResolvedValue([
      { code: '000001', name: '基金A', buyNetValue: 1, shares: 100, buyDate: '2024-01-01' },
    ])
    const { useHoldingStore } = await import('@/stores/holding')
    const store = useHoldingStore()
    await store.initHoldings()
    expect(store.holdings.length).toBeGreaterThanOrEqual(0)
  })

  test('initHoldings 清理旧字段', async () => {
    mockGetHoldings.mockResolvedValue([
      { code: '000001', name: '基金A', buyNetValue: 1, shares: 100, buyDate: '2024-01-01', shareClass: 'A', serviceFeeRate: 0.5 },
    ])
    const { useHoldingStore } = await import('@/stores/holding')
    const store = useHoldingStore()
    await store.initHoldings()
    expect(store.holdings[0]?.shareClass).toBeUndefined()
  })
})
